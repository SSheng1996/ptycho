...
(default = 1)
   ...
   # Partial-pixel shift
       ...
       CC[(m-npy.int(npy.fix(m/2))):(m+1+npy.int(npy.fix((m-1)/2))),(n-npy.int(npy.fix(n/2))):(n+1+npy.int(npy.fix((n-1)/2)))] = (sf.fftshift(buf1ft)*npy.conj(sf.fftshift(buf2ft)))[:,:]
...
def subpixel_align(ref,tmp,threshold_flag,threshold, subpixel_flag):
    ...
    if npy.size(ndims) == 3:
       ...
       return sf.ifftshift(sf.fftn(sf.fftshift(tmp_fft_new))),x_shift,y_shift, z_shift
...
def pixel_shift_1d(array,x_shift):
    nx = npy.size(array)
    tmp = sf.ifftshift(sf.ifftn(sf.fftshift(array)))
    nest = npy.arange(nx)
    tmp = tmp * npy.exp(1j*2*npy.pi*(x_shift*(nest-nx/2.)/(nx)))
    return sf.ifftshift(sf.fftn(sf.fftshift(tmp)))
